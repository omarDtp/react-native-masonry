...
export const assignObjectColumn = (nColumns, index, targetObject) => ({
  ...targetObject,
...
export const assignObjectIndex = (index, targetObject) => ({
  ...targetObject,
  ...{ index }
});

// findMinIndex :: [Numbers] -> Number
export const findMinIndex = srcArray =>
  srcArray.reduce(
    (shortest, cValue, cIndex, cArray) =>
      cValue < cArray[shortest] ? cIndex : shortest,
    0
  );

// containMatchingUris :: ([brick], [brick]) -> Bool
export const containMatchingUris = (r1, r2) =>
  isEqual(r1.map(brick => brick.uri), r2.map(brick => brick.uri));
    ...
    bricks
      ...
      .map(resolveTask =>
        resolveTask.fork(
          ...
          resolvedBrick => {
            this.setState(state => {
              const sortedData = this._insertIntoColumn(
                resolvedBrick,
                state._sortedData
              );
              return {
  ...
  _insertIntoColumn = (resolvedBrick, dataSet) => {
    ...
    switch (priority) {
      case "balance":
        ...
        const newColumnHeights =
          heightsCopy[columnIndex] + resolvedBrick.dimensions.height;
